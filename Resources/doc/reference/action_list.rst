...
Configure the default ordering in the list view (using multiple columns)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

It is also possible to predefine the initial ordering for columns, before any filters have been 
applied and using multiple column types. For example, it is common for a Product entity to have an associated Category 
entity. For this example, when the Product list view is first displayed, the Product's initial ordering has been 
set up such that the Products are listed by ascending Category name first, then followed by the ascending 
Product name second and then finally by the descending Product Id. This can be achieved by 
overriding the createQuery method within the ProductAdmin.php class as shown below.

.. code-block:: php

    public function createQuery($context = 'list')
    {
        // Check to see whether the filter has been set
        $filter = $this->getRequest()->query->get('filter') ? 1 : 0;
        
        if( $filter ) {
            // If the filter has been set, filter the results as normal
            $query = parent::createQuery($context);       
        }
        else {
            // The filter has not been set, so create an initial query for the list view
            ...
            $qb->select( array( 'p', 'c' ))
                                 ...
                                 ->orderby('c.name ASC, p.name ASC, p.id DESC'); // Set the initial ordering here
...
The createQuery function now provides a custom, multiple column ordered query when no filters have been set. 
However, if the filter parameter is set within the URL query string then the filter is applied as normal, 
returning the query generated by the default parent classe's createQuery method. 
